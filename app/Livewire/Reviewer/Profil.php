...
class Profil extends Component
{
    ...
    public function render()
    {
        ...
        $totalReviews = $completed->count();

        $avgDays = $totalReviews > 0
            ? round($completed->avg(fn($a) => $a->date_assigned->diffInDays($a->date_completed)))
            : null;

        $totalInvited = ReviewAssignment::where('reviewer_id', $userId)->count();

        $totalAccepted = ReviewAssignment::where('reviewer_id', $userId)
            ->whereIn('status', ['accepted', 'completed'])
            ->count();

        $acceptanceRate = $totalInvited > 0
            ? round(($totalAccepted / $totalInvited) * 100)
            : null;

        $currentAvatar = auth()->user()->avatar;

        return view('livewire.reviewer.profil', compact(
            'totalReviews', 'avgDays', 'acceptanceRate', 'totalInvited', 'currentAvatar'
        ))->title('Profil Saya — Panel Reviewer');
    }
}
